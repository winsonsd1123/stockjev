...
export default function HomePage() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8">
          ...
          <div className="min-w-[200px] flex-1">
            ...
            <div className="h-2 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
              <div
                ...
                style={{ width: `${discoverBusy || discoverProgress.total > 0 ? discoverBar : 0}%` }}
              />
            </div>
          </div>
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <h2 className="mb-3 text-lg font-medium">盘中判定</h2>
        <p className="mb-2 text-xs text-zinc-500">
          交易日 9:30–15:00，页面打开时每 15 分钟自动判定观察池买入 / 持仓卖出
        </p>
        {(pollBusy || pollProgress.total > 0 || pollProgress.processed > 0) && (
          <div className="mb-3">
            <div className="mb-1 flex justify-between text-xs text-zinc-500">
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
              <div
                className={`h-full bg-sky-500 transition-all ${
                  pollBusy && pollProgress.total === 0 ? "animate-pulse" : ""
                }`}
                style={{
        ...
        <div
          ref={pollLogRef}
          className="max-h-56 overflow-y-auto rounded-lg bg-zinc-50 px-3 py-2 font-mono text-xs leading-5 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300"
        >
          {pollLog.length === 0 ? (
            <div className="text-zinc-400">
              {pollBusy ? "等待盘中判定本步返回…" : "尚无盘中判定记录"}
            </div>
          ) : (
            pollLog.map((line, i) => (
              <div
